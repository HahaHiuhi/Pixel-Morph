...
// ===== UI =====
(document.getElementById("img1") as HTMLInputElement).addEventListener("change", async (e) => {
  const file = (e.target as HTMLInputElement).files?.[0];
  if (!file) return;
  const img = await loadImage(file);
  (document.getElementById("name1") as HTMLElement).textContent = file.name.slice(0, 18);
  (document.getElementById("label1") as HTMLElement).classList.add("chosen");
  ctx.drawImage(img, 0, 0, SIZE, SIZE);
});

(document.getElementById("img2") as HTMLInputElement).addEventListener("change", (e) => {
  const file = (e.target as HTMLInputElement).files?.[0];
  if (!file) return;
  (document.getElementById("name2") as HTMLElement).textContent = file.name.slice(0, 18);
  (document.getElementById("label2") as HTMLElement).classList.add("chosen");
});
...
interface MorphData {
  ...
  finalBuffer: Uint8ClampedArray;
}

function buildMorphData(data1: Uint8ClampedArray, data2: Uint8ClampedArray): MorphData {
  const N = SIZE * SIZE;
...
// ===== DRAW =====
function draw(frame: number, { startPos, endPos, colors, finalBuffer }: MorphData): void {
  const N = SIZE * SIZE;
...
function recordAnimation(morphData: MorphData): MediaRecorder {
  const chunks: Blob[] = [];
...
// ===== ANIMATE =====
function animate(frame: number, morphData: MorphData, recorder?: MediaRecorder): Promise<void> {
...
document.getElementById("form")!.addEventListener("submit", async (e) => {
  ...
  const f1 = (document.getElementById("img1") as HTMLInputElement).files![0];
  const f2 = (document.getElementById("img2") as HTMLInputElement).files![0];
